...
public class LevelGenerator : MonoBehaviour
{
    ...
    private Chunk _starterTemplate;
    ...
    void Awake()
    {
        ...
        if (starterChunkPrefab != null)
        {
            _starterTemplate = starterChunkPrefab.GetComponent<Chunk>();

            if (_starterTemplate != null && !_pools.ContainsKey(_starterTemplate))
                _pools[_starterTemplate] = new ObjectPool<Chunk>(_starterTemplate, transform, 1);
        }
    }
    ...
    void Start()
    {
        if (_starterTemplate != null)
            SpawnSpecificChunk(_starterTemplate);
        else
            SpawnNextChunk();
